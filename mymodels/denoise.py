...
class DenoiseNet(nn.Module):
    ...
    def get_supervised_loss(self, pcl_noisy, pcl_clean):
        """
        Denoising score matching with multi-scale feature extraction.
        Args:
        ...
        # Define multiple scales (example: using two scales for multi-scale extraction)
        scale_values = [self.frame_knn, self.frame_knn * 2]  # Two different K values for multi-scale

        grad_preds = []  # To store predictions from different scales
        for K in scale_values:
            # Local frame construction at each scale
            _, _, frames = pytorch3d.ops.knn_points(pcl_noisy[:,pnt_idx,:], pcl_noisy, K=K, return_nn=True)  # (B, n, K, 3)
            ...
            _, _, clean_nbs = pytorch3d.ops.knn_points(
                frames.view(-1, K, d),    # (B*n, K, 3)
                pcl_clean.unsqueeze(1).repeat(1, len(pnt_idx), 1, 1).view(-1, N_clean, d),   # (B*n, M, 3)
                K=self.num_clean_nbs,
                return_nn=True,
            )   # (B*n, K, C, 3)
            clean_nbs = clean_nbs.view(B, len(pnt_idx), K, self.num_clean_nbs, d)  # (B, n, K, C, 3)

            # Noise vectors
            noise_vecs = frames.unsqueeze(dim=3) - clean_nbs  # (B, n, K, C, 3)
            noise_vecs = noise_vecs.mean(dim=3)  # (B, n, K, 3)

            # Denoising score matching for the current scale
            grad_pred = self.score_net(
                x=frames_centered.view(-1, K, d),
                c=feat.view(-1, F),
            ).reshape(B, len(pnt_idx), K, d)   # (B, n, K, 3)
            
            grad_preds.append(grad_pred)

        # Combine gradients from multiple scales (e.g., by averaging)
        grad_pred_multi_scale = torch.stack(grad_preds).mean(dim=0)  # (B, n, K, 3)

        # Target gradients from noise vectors
        grad_target = - 1 * noise_vecs   # (B, n, K, 3)

        # Compute the loss using multi-scale gradients
        loss = 0.5 * ((grad_target - grad_pred_multi_scale) ** 2.0 * (1.0 / self.dsm_sigma)).sum(dim=-1).mean()
